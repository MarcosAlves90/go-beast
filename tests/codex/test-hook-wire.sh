...
assert_symlink_target \
  ...
  "codex live rewire replaces managed hook symlink"

assert_symlink_target \
  "$TEST_HOME/.codex/hooks/code-verify-flag.sh" \
  "$(cd "$EXTERNAL_HOOKS" && pwd -P)/code-verify-flag.sh" \
  "codex live rewire preserves external hook symlink"
...
assert_contains \
  ...
  "codex live rewire preserves external config entries"

assert_contains \
  "$OUTPUT_JSON" \
  "\"status\": \"replaced\"" \
  "codex live rewire reports replaced managed hook"

assert_contains \
  "$OUTPUT_JSON" \
  "\"status\": \"warn\"" \
  "codex live rewire reports preserved external hook"
