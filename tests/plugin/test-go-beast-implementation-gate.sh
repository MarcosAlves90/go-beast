...
mkdir -p "$PROJECT_ROOT/.go-beast" "$PROJECT_ROOT/src"
...
write_state() {
  ...
  cat > "$STATE_FILE" <<EOF
{"version":1,"session_id":"runtime-gate-test","cwd":"$PROJECT_ROOT","harness":"codex","mode":"bootstrap","active_beast":"go-hawk","required_artifact":".go-beast/REQUIREMENTS.md","implementation_unlocked":$implementation_unlocked,"task_state":"$task_state","approval_state":"$approval_state","task_id":"runtime-gate-test","unanchored_stop_count":0,"last_reanchor_reason":"","updated_at":"2026-09-11T00:00:00Z"}
EOF
}
...
run_gate() {
  ...
  local file_path="$2"
  local input
  input=$(jq -nc --arg file_path "$file_path" --arg cwd "$PROJECT_ROOT" '{tool_name:"Edit",tool_input:{file_path:$file_path,new_string:"implementation"},session_id:"runtime-gate-test",cwd:$cwd}')
...
run_bash_gate() {
  ...
  local command="$2"
  local input
  input=$(jq -nc --arg command "$command" --arg cwd "$PROJECT_ROOT" '{tool_name:"Bash",tool_input:{command:$command},session_id:"runtime-gate-test",cwd:$cwd}')
  printf '%s' "$input" | HOME="$TEST_HOME" GO_BEAST_STATE_DIR="$STATE_DIR" GO_BEAST_HARNESS_OVERRIDE="$harness" bash "$GATE" 2>&1
}
...
output=$(run_gate codex "$PROJECT_ROOT/src/new-file.mjs")
status=$?
...
output=$(run_gate codex "$PROJECT_ROOT/.go-beast/REQUIREMENTS.md")
...
output=$(run_gate codex "$PROJECT_ROOT/src/new-file.mjs")
[[ "$?" -eq 2 ]] || fail "unverified completed task remains blocked"
pass "unverified completed task remains blocked"

write_state true complete
output=$(run_gate codex "$PROJECT_ROOT/src/new-file.mjs")
[[ "$?" -eq 0 ]] || fail "authorized completed task passes"
pass "authorized completed task passes"
...
output=$(run_gate copilot "$PROJECT_ROOT/src/new-file.mjs")
...
output=$(run_bash_gate codex "printf 'requirements' > '$PROJECT_ROOT/.go-beast/REQUIREMENTS.md'")
