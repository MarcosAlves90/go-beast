...
write_state() {
  ...
  printf '%s\n' "{\"version\":1,\"session_id\":\"shell-gate-test\",\"cwd\":\"$REPO_ROOT\",\"harness\":\"codex\",\"mode\":\"bootstrap\",\"active_beast\":\"go-hawk\",\"required_artifact\":\".go-beast/REQUIREMENTS.md\",\"implementation_unlocked\":$implementation_unlocked,\"task_state\":\"$task_state\",\"task_id\":\"shell-gate-test\",\"unanchored_stop_count\":0,\"last_reanchor_reason\":\"\",\"updated_at\":\"2026-09-11T00:00:00Z\"}" > "$STATE_FILE"
}
...
run_gate() {
  ...
  local command="$2"
  local input
  input=$(jq -nc --arg command "$command" --arg cwd "$REPO_ROOT" '{tool_name:"Bash",tool_input:{command:$command},session_id:"shell-gate-test",cwd:$cwd}')
...
output=$(run_gate codex "printf 'requirements' > '$REPO_ROOT/.go-beast/REQUIREMENTS.md'")
